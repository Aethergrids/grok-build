...
/// ZDR/privacy: xAI client-identity headers go only to first-party endpoints,
...
impl ImageGenConfig {
    ...
    /// value is never overwritten. No-op when `Disabled`, and no-op when the
    /// base URL is not first-party (see [`is_first_party_base_url`]).
...
mod tests {
    ...
    /// ZDR: `xai_api_base_url` is user-overridable, so the session-id identity
    /// header must not be stamped for a non-xAI (enterprise / third-party) base.
    #[test]
    fn stamp_session_id_header_skips_non_first_party_base_url() {
        let mk = |base_url: &str| ImageGenConfig::Enabled {
            api_key: "k".into(),
        ...
        for base in [
            ...
            cfg.stamp_session_id_header("sess-123");
            assert!(
                hdrs(&cfg).get(SESSION_ID_HEADER).is_none(),
                "identity header leaked to non-first-party base: {base}"
            );
        }
        ...
        for base in ["https://api.x.ai/v1", "https://x.ai/v1"] {
            ...
            assert_eq!(
                hdrs(&cfg).get(SESSION_ID_HEADER).map(String::as_str),
                Some("sess-123"),
                "first-party base should still be stamped: {base}"
            );
        }
    }
