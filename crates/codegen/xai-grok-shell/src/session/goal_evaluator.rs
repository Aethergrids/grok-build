...
/// `endpoint_is_first_party` gates the `x-grok-*` identity fields: this is a
/// side-query that rides whatever sampler the session resolved, which may be a
/// user-configured third-party provider. xAI identity (session / agent id)
/// never leaves for a non-first-party endpoint. Mirrors the permission
/// classifier's gating in `sampler_turn.rs`.
pub(crate) fn build_goal_evaluator_request(
    ...
    ConversationRequest {
        ...
        x_grok_conv_id: endpoint_is_first_party.then(|| session_id.to_owned()),
        x_grok_req_id: endpoint_is_first_party
            .then(|| format!("xai-goal-eval-{}", uuid::Uuid::new_v4())),
        x_grok_session_id: endpoint_is_first_party.then(|| session_id.to_owned()),
        x_grok_agent_id: endpoint_is_first_party.then(xai_grok_telemetry::id::agent_id),
...
mod tests {
    ...
    fn request_is_tool_free_and_schema_constrained() {
        let request =
            build_goal_evaluator_request("goal", "trace", None, "small".into(), "s", true);
    ...
    /// ZDR: the evaluator rides the session's sampler, which may be a
    /// user-configured third-party provider — xAI identity must not go with it.
    #[test]
    fn identity_fields_gated_on_first_party_endpoint() {
        let first_party =
            build_goal_evaluator_request("goal", "trace", None, "small".into(), "s", true);
        assert_eq!(first_party.x_grok_conv_id.as_deref(), Some("s"));
        assert_eq!(first_party.x_grok_session_id.as_deref(), Some("s"));
        assert!(first_party.x_grok_req_id.is_some());
        assert!(first_party.x_grok_agent_id.is_some());

        let third_party =
            build_goal_evaluator_request("goal", "trace", None, "small".into(), "s", false);
        assert!(third_party.x_grok_conv_id.is_none());
        assert!(third_party.x_grok_session_id.is_none());
        assert!(third_party.x_grok_req_id.is_none());
        assert!(third_party.x_grok_agent_id.is_none());
        // The actual inference payload is unaffected by the gate.
        assert_eq!(third_party.model.as_deref(), Some("small"));
        assert_eq!(third_party.items.len(), first_party.items.len());
    }
}
